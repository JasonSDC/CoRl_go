#!/usr/bin/env python3
"""Flat-ground speed check with fixed high-level command vx=1.0."""
...
parser = argparse.ArgumentParser(description="Run flat-ground fixed high-level command and print actual speed.")
parser.add_argument("--task", type=str, default="Isaac-Velocity-Rough-Unitree-Go2-v0")
parser.add_argument("--num_envs", type=int, default=1)
parser.add_argument("--terrain", type=str, default="flat", choices=["flat"])
...
parser.add_argument(
    ...
    type=str,
    default="/home/dacheng.shen/Desktop/omniverse/avoid_obstacles/weights/model_7850.pt",
)
...
def main() -> None:
    ...
    fixed_cmd = [float(args_cli.cmd_vx), float(args_cli.cmd_vy), float(args_cli.cmd_wz)]
    print(
        f"[INFO] Flat mode fixed velocity command (no high-level policy): "
        ...
        with torch.inference_mode():
            ...
            if args_cli.print_interval > 0 and step_idx % int(args_cli.print_interval) == 0:
                ...
                print(
                    ...
                    f"target_cmd=({fixed_cmd[0]:+.2f}, {fixed_cmd[1]:+.2f}, {fixed_cmd[2]:+.2f}) "
                    f"actual_lin_vel=({root_lin[0]:+.2f}, {root_lin[1]:+.2f}, {root_lin[2]:+.2f})"
                )
