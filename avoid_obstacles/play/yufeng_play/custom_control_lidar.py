#!/usr/bin/env python3
"""Walk with legacy model_1500 + render lidar above robot."""
...
parser = argparse.ArgumentParser(description="Combined walking + lidar render demo.")
...
# custom_rl_env expects this module.
...
def _adapt_legacy_agent_cfg(cfg: dict) -> dict:
    ...
    algo_cfg = adapted.setdefault("algorithm", {})
...
def _build_legacy_joint_mapping(base_env) -> torch.Tensor | None:
    ...
    name_to_idx = {name: idx for idx, name in enumerate(sim_joint_names)}
    missing = [name for name in LEGACY_GO2_JOINT_ORDER if name not in name_to_idx]
    if missing:
        print(f"[WARN] Legacy remap disabled. Missing joints: {missing}")
        return None
    mapping = torch.tensor([name_to_idx[name] for name in LEGACY_GO2_JOINT_ORDER], dtype=torch.long, device=base_env.device)
...
def _setup_rtx_lidar(base_env):
    ...
    for env_id in range(base_env.num_envs):
        sensor = LidarRtx(
            f"/World/envs/env_{env_id}/Robot/base/lidar_sensor_custom",
...
def main() -> None:
    print(f"[INFO] Using fixed walking checkpoint: {LEGACY_WALK_CHECKPOINT}")
    ...
    if low_actor_in_dim is not None:
        base_obs_dim = int(base_env.get_observations()["policy"].shape[-1])
        if base_obs_dim > low_actor_in_dim:
            print(f"[INFO] Truncating policy obs: {base_obs_dim} -> {low_actor_in_dim}")
    ...
    print(
        "[INFO] Start combined render: "
        f"target_cmd_xyz=({args_cli.walk_cmd_x:.3f}, {args_cli.walk_cmd_y:.3f}, {args_cli.walk_cmd_z:.3f}), "
        f"lidar_bins={args_cli.lidar_bins}, obstacle_threshold={args_cli.obstacle_distance_threshold:.2f}m"
    )
    step_idx = 0
